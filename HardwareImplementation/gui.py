...
from KNN.KNN_model import KNN_model
...
class CustomLabel(QLabel):
    ...
    def paintEvent(self, event):
        ...
        rect = self.rect().adjusted(self.padding // 2,
                                    self.padding // 2,
                                    -self.padding // 2,
                                    -self.padding // 2)
        path = QPainterPath()
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.setStyleSheet("""
            QWidget#MainWindow { background: qlineargradient(x1:0,y1:0,x2:0,y2:1, stop:0 #eaf6ff, stop:1 #b3e0ff); font-family:'Segoe UI',sans-serif; }
            QLabel { font-size:20px; color:#333; }
            QComboBox { font-size:20px; padding:6px 12px; border-radius:8px; font-weight:bold; background-color:#77b9f7; color:black; border:2px solid #000; }
            QComboBox:hover { background-color:#45a1f7; }
            QComboBox::down-arrow { image:url(utils/arrow.png); width:12px; height:12px; }
            QPushButton { font-size:20px; padding:6px 12px; border-radius:8px; font-weight:bold; border:2px solid #000; background-color:#77b9f7; color:black; }
    ...
    def _build_main_page(self):
        ...
        self.model_combo.addItems(["KNN_RDCSD", "LSTM"])
        le = self.model_combo.lineEdit(); le.setAlignment(Qt.AlignCenter); le.setReadOnly(True)
        ...
        self.status_label = QLabel("Status: Idle"); self.status_label.setAlignment(Qt.AlignCenter)
        self.predicted_label = CustomLabel("Prediction"); self.predicted_label.setAlignment(Qt.AlignCenter)
        self.predicted_label.setStyleSheet("font-size:50px;color:#77b9f7;")

        top_layout = QHBoxLayout()
        top_layout.setAlignment(Qt.AlignCenter)
        top_layout.addWidget(self.browse_button); top_layout.addWidget(self.plot_button)
        model_layout = QHBoxLayout(); model_layout.addWidget(self.model_combo)
        ...
        self.main_page = QWidget(); self.main_page.setLayout(main_layout)
    ...
    def _build_plot_page(self):
        ...
        for label_text, default, attr_name in [
            ...
            s.valueChanged.connect(
                partial(self._on_cutoff_changed, attr_name, val)
            )
        ...
        for label_text, default, attr_name in [
            ...
            s.valueChanged.connect(
                partial(self._on_psd_changed, attr_name, val)
            )
    ...
    def plot_graph(self):
        ...
        timesteps = range(0, data.shape[1])
    ...
    def _connect_plot_button(self):
        if self.selected_path is not None:
            ...
            self.update_progress("No data path selected")
    ...
    def update_progress(self, state: str):
    ...
    def resizeEvent(self, event):
        ...
        if self.predicted_label.text():
            font = QFont("Arial", 40, QFont.Bold)
            self.predicted_label.setFont(font)
            fm = QFontMetrics(font)
            rect = fm.boundingRect(self.predicted_label.text())
            w = rect.width() + self.predicted_label.padding * 10
            h = rect.height() + self.predicted_label.padding * 10
            self.predicted_label.setFixedSize(w, h)
            self.predicted_label.setStyleSheet("font-size:40px;color:#333;")
    ...
    def on_start(self):
        if not self.selected_path:
            self.update_progress("No data selected")
            return
        self.extract_data()
        self.processing()
        self.predict_word()
    ...
    def extract_data(self):
        self.update_progress("Extracting")
        print("Extracting")
